...
alloc_proc(void) {
    ...
    memset(proc->name, 0, PROC_NAME_LEN);
     //LAB5 2213124 : (update LAB4 steps)
     /*
...
proc_run(struct proc_struct *proc) {
    if (proc != current) {
        ...
        */
        // 禁用中断
        bool intr_flag;
        struct proc_struct *prev = current, *next = proc; // prev指向当前正在运行的进程
